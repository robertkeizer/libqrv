...
const LibQRV = function( _config, cb ){
	...
	async.waterfall( [ ( cb ) => {
		...
		const qrOptions = Joi.object( ).keys( {
			width: Joi.number( ).default( 240 ),
			scale: Joi.number( ).default( 1 ),
			margin: Joi.number( ).default( 0 ),
			errorCorrectionLevel: Joi.string( ).default( "low" )
		} );

		const videoOutputOptions = Joi.object( ).keys( {
			framerate: Joi.number( ).default( 25 ),
		} );

		const videoInputOptions = Joi.object( ).keys( {
			framerate: Joi.number( ).default( 1 )
		} );

		const videoOptions = Joi.object( ).keys( {
			output: videoOutputOptions.default(videoOutputOptions.validate({}).value ),
			input: videoInputOptions.default(videoInputOptions.validate({}).value )
		} );

		Joi.validate( _config, Joi.object( ).keys( {
			outputDirectory: Joi.string( ).default( ( ) => {
				return tmp.dirSync( ).name;
			}, "Uses tmp.dirSync node module." ),
			debug: Joi.boolean( ).default( false ),
			qr: qrOptions.default( qrOptions.validate({}).value),
			video: videoOptions.default(videoOptions.validate({}).value)
		} ), cb );
...
LibQRV.prototype._encode = function( data, opts, cb ){
	...
	*/
	...
	const _options = {
		...
		scale: this.config.qr.scale,
		width: this.config.qr.width,
...
LibQRV.prototype._handleReadable = function( _readable, cb ){
	...
	async.waterfall( [ ( cb ) => {
		...
		TODO make this an option in the configs, so that we
		are not manipulating the binary unless we're told to.
		...
		*/
		...
		TODO similar to the header; This should be made
		to be configurable.
		...
		*/
		...
		const ffmpegCmd = new FfmpegCommand( _opts.tmpDir + "/" + _opts.baseName + "-%d.png" ).inputOptions( [
			"-r " + this.config.video.input.framerate,
			"-f image2",
		] ).outputOptions( [
			"-c:v libx264",
			"-r " + this.config.video.output.framerate,
			"-pix_fmt yuv420p"
		] ).output( _opts.tmpDir + "/output.mp4" );
...
LibQRV.prototype._doneReadable = function( filenameToUse, _path, cb ){
	...
	const _dest = path.join( this.config.outputDirectory, filenameToUse + ".mp4" );

	fs.copyFile( _path, _dest, ( err ) => {
		if( err ){ return cb( err ); }
...
// There are common use cases that we'd like to abstract
// away, allowing users of this library to not be bothered
// with the implementation specifics of how LibQRV is set up.
