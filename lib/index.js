...
LibQRV.prototype._handleReadable = function( _readable, cb ){
	async.waterfall( [ ( cb ) => {
		...
		console.log( "This is _handleReadable; I have _readable of " );
		console.log( _readable) ;
		...
		async.whilst( ( cb ) => {
			return cb( null, _readable.readable && !_readable.readableEnded );
		}, ( cb ) => {
			
			if( _readable.isPaused() ){
				_readable.resume();
			}
